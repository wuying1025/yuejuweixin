...
class Wxpay extends CI_Controller {
	...
	function do_pay(){
		...
		$input->SetTotal_fee($yueju_order->price*100);//$order->price
		$input->SetTime_start(date("YmdHis"));
		$input->SetTime_expire(date("YmdHis", time() + 600000));
		$input->SetGoods_tag($yueju_order->title);
	...
	function yueju_notify(){
		...
		if($array!=null)
		{
			...
			if ($payinfo) {
				...
				if($rs>0)
				{//告知微信我成功了
					$this->send_success_msg($out_trade_no);

...
class Order extends CI_Controller
{
    ...
    public function house_order()
    {
        ...
        $start_time = htmlspecialchars($this->input->post('startDate'));
        $end_time = htmlspecialchars($this->input->post('endDate'));
    ...
    public function confirm_order()
    {
        ...
        $startTime = htmlspecialchars($this->input->post('startDate'));
        $endTime = htmlspecialchars($this->input->post('endDate'));
        $days = floor((strtotime($endTime) - strtotime($startTime)) / 86400);
        $realName = htmlspecialchars($this->input->post('name'));
        $phone = htmlspecialchars($this->input->post('tel'));
    ...
    public function add_comment()
    {
        ...
        $val = htmlspecialchars($this->input->get("val"));
        $score = htmlspecialchars($this->input->get("score"));
        $clean_score = htmlspecialchars($this->input->get("cleanScore"));
        $traffic_score = htmlspecialchars($this->input->get("trafficScore"));
        $manage_score = htmlspecialchars($this->input->get("manageScore"));
        $facility_score = htmlspecialchars($this->input->get("facilityScore"));
